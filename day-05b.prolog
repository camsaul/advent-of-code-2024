...
%!  sort_before_all(X, List) is det.
%
%   A relation such that X should be sorted before every item in List.
sort_before_all(_X, []).

sort_before_all(X, [Y | More]) :-
    \+ rbefore(Y, X),
    sort_before_all(X, More).
...
sorted_list([X, Y | More]) :-
    rbefore(X, Y),
    sort_before_all(X, More), % Make sure there are no items in More that should be sorted before X.
    sorted_list([Y | More]).

compare_before(Order, X, Y) :-
    rbefore(X, Y)
->  Order = '<'
;   Order = '>'.

sorted_list(List, SortedList) :- predsort(compare_before, List, SortedList).

unsorted_lists(Lists, SortedLists) :- exclude(sorted_list, Lists, SortedLists).
...
run(Lists, Sum) :-
    unsorted_lists(Lists, UnsortedLists),
    maplist(sorted_list, UnsortedLists, SortedLists),
