...
trail(Size, Board, Trail) :-
    trail_ending_at(Size, Board, _EndPosition, 9, Trail).

% solve(example, N).
% solve(actual, N).
solve(Source, N) :- init(Source, Size, Board), findall(Trail, trail(Size, Board, Trail), Trails), length(Trails, N).
